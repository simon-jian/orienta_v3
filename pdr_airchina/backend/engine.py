...
class PdrEngine:
    def __init__(self, config: Optional[PdrConfig] = None):
        ...
        self.state = PdrState()
        corridor_env = os.environ.get("PDR_CORRIDORS_PATH", "").strip()
        corridor_file = Path(corridor_env).expanduser() if corridor_env else Path(__file__).resolve().parents[1] / self.cfg.corridor_relpath
    ...
    def reset(self, t_ms: float, initial_heading_deg: Optional[float] = None) -> None:
        self.state = PdrState(recording_start_ms=t_ms)
        self.state.map_match_ready = self.matcher.ready
        if isinstance(initial_heading_deg, (int, float)) and math.isfinite(initial_heading_deg):
            h = wrap_deg(float(initial_heading_deg))
            self.state.heading_fused_deg = h
            self.state.heading_gyro_rad = math.radians(h)
            # Mark heading initialized so the next gyro integration uses this seed.
    ...
    def _step_detect(self, frame: Dict, t_ms: float) -> bool:
        ...
        min_period_ok = is_first or dt >= min_period
        continuous_step = (not is_first) and dt <= self.cfg.max_period_ms

        self.state.rising = self.state.step_sig > self.state.last_sig
        self.state.last_sig = self.state.step_sig
        if not (is_peak and min_period_ok):
            return False
        ...
        if continuous_step:
            ...
            self.state.step_intervals.clear()
