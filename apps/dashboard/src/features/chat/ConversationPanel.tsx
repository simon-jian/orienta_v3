...
function MessageBubble({ msg }: { msg: ChatMessage }) {
  ...
  const bgMap: Record<string, string> = {
    admin: "#0a84ff", pax: "#ffffff", agent: "#5856d6", system: "#fff7ed",
  };
  const colorMap: Record<string, string> = {
    admin: "#ffffff", pax: "#111827", agent: "#ffffff", system: "#7c2d12",
  };
  const labelMap: Record<string, string> = {
    admin: "Operator", pax: "Passenger", agent: "🤖 AI Agent", system: "⚙️ System",
  };
  return (
...
export default function ConversationPanel({
  ...
  const shellStyle: React.CSSProperties = mode === "docked"
    ? {
        ...
        background: "#f2f2f7", borderRadius: 16, boxShadow: "0 8px 32px rgba(0,0,0,0.12)",
        ...
        <div style={{ padding: "10px 12px", borderTop: "1px solid rgba(0,0,0,0.08)", display: "flex", gap: 8, flexDirection: "column" }}>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {quickReplies.map((q, i) => (
              <button key={i} className="btn" style={{ fontSize: 10, padding: "2px 7px" }} onClick={() => onSend(q)}>
