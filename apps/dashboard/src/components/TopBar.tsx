...
export default function TopBar(props: {
  ...
  indoorMapFilters?: React.ReactNode;
}) {
  const {
    search, onSearch,
    title = "中国国际航空公司后台",
    ...
    transferCount, transferUrgentCount,
    dataSource, userLabel, onLogout,
    extraRight, onPaxClick, onUrgentClick,
    mapViewFilter = "all",
    indoorMapFilters,
  } = props;
    ...
    <div className="topbar">
      ...
      <div className="search">
        ...
        <span className={"chip " + (mapViewFilter === "all" ? "active" : "")}>
          <button onClick={onPaxClick} title="Show all passengers">
            {passengerCount > 0 ? `Pax: ${passengerCount}` : "Pax: 0"}
          </button>
        </span>
      ...
      <div className="topbarRight">
        ...
        {extraRight}
        {onLogout ? (
          <button className="btn" onClick={onLogout} title="退出登录">退出</button>
        ) : null}
      </div>
    </div>
  );
