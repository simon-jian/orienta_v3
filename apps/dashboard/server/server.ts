...
console.log(`Passenger registry: ${DB_PATH}`);
...
registerPaxSessionRoutes(paxSessionRouter, registry, accountStore);
app.use("/api/pax", paxRateLimit);
app.use("/api/pax", paxSessionRouter);

const pushRouter = express.Router();
registerPushRoutes(pushRouter, store);
...
registerPassengerRoutes(passengerRouter, store, registry);
...
if (INDOOR_MAP_UPSTREAM) {
  ...
  console.log("Indoor map proxy: /indoor-map →", INDOOR_MAP_UPSTREAM);
  ...
  console.log("Indoor map tiles: bundled from", LOCAL_INDOOR_MAP_TILES_DIR);
}
...
if (INDOOR_MAP_API_UPSTREAM) {
  ...
  console.log("Indoor map API proxy: /indoor-map-api →", INDOOR_MAP_API_UPSTREAM);
  ...
  console.log("Indoor map API: bundled from", LOCAL_INDOOR_MAP_API_DIR);
}
...
if (PDR_API_ORIGIN) {
  ...
  console.log("PDR API proxy: /pdr-api →", PDR_API_ORIGIN);
} else {
...
attachWsHub(server, store, registry);
startMaintenanceJobs({ registry, chatRepo });

loadPoiCache().then(() => {
  server.listen(PORT, "0.0.0.0", () => {
    console.log(`Orienta v3 server listening on http://0.0.0.0:${PORT}`);
